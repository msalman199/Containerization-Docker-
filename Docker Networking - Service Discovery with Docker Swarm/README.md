...
# 🔗 Docker Networking - Service Discovery with Docker Swarm
...
![DNS](https://img.shields.io/badge/DNS-Resolution-4B32C3?style=for-the-badge&logo=internetcomputer&logoColor=white)
...
**A hands-on lab in cluster orchestration, DNS-based service discovery, and fault-tolerant microservice communication**
...
- [🧩 Task 1: Create a Docker Swarm Cluster and Deploy Services with Dynamic Ports](#-task-1-create-a-docker-swarm-cluster-and-deploy-services-with-dynamic-ports)
- [🔍 Task 2: Set Up Service Discovery for Services to Communicate](#-task-2-set-up-service-discovery-for-services-to-communicate)
- [🌐 Task 3: Explore DNS Resolution in Docker Swarm](#-task-3-explore-dns-resolution-in-docker-swarm)
- [📡 Task 4: Test Service Discovery with API Calls Between Containers](#-task-4-test-service-discovery-with-api-calls-between-containers)
- [🛡️ Task 5: Implement Fault Tolerance and Automatic Recovery](#️-task-5-implement-fault-tolerance-and-automatic-recovery)
...
| # | Objective |
|---|-----------|
| 1 | 🐳 Create and manage a Docker Swarm cluster for container orchestration |
| 2 | 🔀 Deploy services with dynamic port allocation in Docker Swarm |
| 3 | 🔗 Configure and implement service discovery mechanisms for inter-service communication |
| 4 | 🌐 Understand and utilize DNS resolution within Docker Swarm networks |
| 5 | 📡 Test service discovery by establishing API communication between containers |
| 6 | 🛡️ Implement fault tolerance and automatic recovery mechanisms for failed services |
| 7 | 📊 Monitor and troubleshoot service discovery issues in a distributed environment |
...
| 🌐 Networking | Knowledge of IP addresses, ports, and DNS concepts |
| 🔌 REST APIs | Understanding of REST APIs and HTTP requests |
| 📄 YAML | Basic knowledge of YAML configuration files |

## ☁️ Ready-to-Use Cloud Machines

> **💡 Note:** Al Nafi provides pre-configured Linux-based cloud machines for this lab. Simply click **Start Lab** to access your environment — no need to build your own VM or install Docker.

Your lab environment includes:

- 🖥️ Ubuntu 20.04 LTS with Docker Engine pre-installed
- 📦 Docker Compose pre-configured
- 🛠️ Network utilities (`curl`, `dig`, `nslookup`) available
- ✍️ Text editors (`nano`, `vim`) for configuration files
...
> **📌 TODO:** Add an environment variable for the log level and extend `/health` to report uptime instead of just status.

### 🔹 Subtask 1.4: Build Docker Images

```bash
# 🏗️ Build web application image
cd ~/swarm-lab/web-app
docker build -t web-app:latest .

# 🏗️ Build API service image
cd ~/swarm-lab/api-service
...
> **📌 TODO:** Capture a `dig` trace before and after killing the debug container, and note whether resolution latency changes.
...
## 🛡️ Task 5: Implement Fault Tolerance and Automatic Recovery
...
# ⏱️ Run monitoring for 1 minute
timeout 60 ./monitor_services.sh
```

> **📌 TODO:** Extend `monitor_services.sh` to log each cycle's connectivity result to a timestamped file for later trend analysis.
...
<details>
<summary><strong>❌ Issue 1: Services cannot resolve each other</strong></summary>
...
<details>
<summary><strong>❌ Issue 2: DNS resolution fails</strong></summary>

```bash
# 🔎 Check Docker daemon DNS settings
docker system info | grep DNS

# ♻️ Restart Docker service if needed
...
<details>
<summary><strong>❌ Issue 3: Health checks failing</strong></summary>

```bash
# 📜 Check health check logs
docker service logs api-service | grep health

# ⚙️ Update health check command
...
- 🛡️ Implemented fault tolerance with health checks, automatic recovery, and service scaling
...
- 🛡️ Creating fault-tolerant distributed systems
