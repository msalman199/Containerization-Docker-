...
# 🚀 Docker and CI/CD - Integrating Docker with GitHub Actions
...
**Lab 65 — Building an automated, tested, and security-scanned Docker CI/CD pipeline with GitHub Actions**
...
- [🏗️ Task 1: Set up a GitHub Repository with a Docker-based Application](#️-task-1-set-up-a-github-repository-with-a-docker-based-application)
- [⚙️ Task 2: Create GitHub Actions Workflow for Docker CI/CD](#️-task-2-create-github-actions-workflow-for-docker-cicd)
- [🐳 Task 3: Configure Docker Hub Integration](#-task-3-configure-docker-hub-integration)
- [✨ Task 4: Add Advanced Pipeline Features](#-task-4-add-advanced-pipeline-features)
- [📊 Task 5: Monitor Pipeline Executions and Troubleshoot Failures](#-task-5-monitor-pipeline-executions-and-troubleshoot-failures)
...
| # | Objective |
|---|-----------|
| 1 | 📦 Set up a GitHub repository with a Docker-based application |
| 2 | ⚙️ Create and configure GitHub Actions workflows for Docker CI/CD |
| 3 | 🐳 Build and push Docker images automatically to Docker Hub |
| 4 | 🧪 Implement automated testing in CI/CD pipelines |
| 5 | 📊 Monitor pipeline executions and troubleshoot common failures |
| 6 | 🔗 Understand the integration between Docker and GitHub Actions for continuous deployment |

## 📋 Prerequisites

| Requirement | Details |
|---|---|
| 🐳 Docker Basics | Basic understanding of Docker concepts and commands |
| 🔀 Git & GitHub | Familiarity with Git and GitHub |
| 📄 YAML | Basic knowledge of YAML syntax |
| 🌐 Web Apps | Understanding of web applications (Node.js basics helpful) |
| 🔑 GitHub Account | Free tier is sufficient |
| 🔑 Docker Hub Account | Free tier is sufficient |

## ☁️ Ready-to-Use Cloud Machines

> **💡 Note:** Al Nafi provides pre-configured Linux-based cloud machines for this lab. Simply click **Start Lab** to access your environment — no need to build your own VM or install additional software!
...
- ✍️ Text editors (`nano`, `vim`) available
- 🛠️ All necessary development tools ready to use

---

## 🏗️ Task 1: Set up a GitHub Repository with a Docker-based Application
...
# ✍️ Create the main application file
nano app.js
```
...
> **📌 TODO:** Add a `/api/users/:id` route and a matching Jest test to practice parameterized routing.
...
```bash
# ✍️ Create a health check script
nano healthcheck.js
```
...
# ✍️ Update package.json to include test script
nano package.json
```
...
3. Name it `docker-cicd-lab`
4. Keep it public
5. Don't initialize with a README (we already have files)
...
## ⚙️ Task 2: Create GitHub Actions Workflow for Docker CI/CD
...
# ✍️ Create the main Docker workflow file
...
> ⚠️ **Important:** Replace `your-dockerhub-username` with your actual Docker Hub username in the `DOCKER_IMAGE` environment variable.
...
> 🛡️ **Security Note:** It's recommended to use Docker Hub access tokens instead of passwords. Create one in Docker Hub **Settings → Security → Access Tokens**.
...
```bash
# ✍️ Create a docker-compose file for local development
...
```bash
# ✍️ Update the workflow file to include deployment
nano .github/workflows/docker.yml
```
...
```bash
# ✍️ Create a comprehensive README file
nano README.md
```
...
1. **Testing** - Runs unit tests and generates coverage
2. **Building** - Builds multi-platform Docker images
3. **Security** - Scans for vulnerabilities
4. **Deployment** - Deploys to staging environment
...
> ⚠️ **Important:** Replace `YOUR_USERNAME` and `YOUR_DOCKERHUB_USERNAME` with your actual usernames.
...
| Area | What to Check |
|---|---|
| 📈 Workflow Overview | Total execution time, success/failure status, individual job status |
| 📜 Job Details | Step-by-step execution logs, error messages and stack traces, resource usage |
| 📦 Artifacts and Outputs | Test results, coverage reports, built Docker images |
...
```bash
# ✍️ Create a separate workflow for PR validation
...
> **📌 TODO:** Add a caching step for `node_modules` in this workflow to speed up repeated PR validation runs.

### 🔹 Subtask 5.5: Set up Monitoring and Notifications

```bash
# ✍️ Create a workflow status notification
...
```bash
# ✍️ Make a small change to trigger the pipeline
nano app.js
```
...
<details>
<summary><strong>❌ Issue 1: Docker Hub Authentication Failure</strong></summary>
...
<details>
<summary><strong>❌ Issue 2: Test Failures in CI</strong></summary>
...
<details>
<summary><strong>❌ Issue 3: Docker Build Failures</strong></summary>
...
<details>
<summary><strong>❌ Issue 4: Workflow Not Triggering</strong></summary>
...
- ⚙️ Set up automated CI/CD pipelines using GitHub Actions that trigger on code changes
...
- ☁️ Integrating with cloud platforms like AWS, Azure, or Google Cloud
