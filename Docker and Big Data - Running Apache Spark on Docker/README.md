...
# 🐳 Lab 49: Docker and Big Data
## Running Apache Spark on Docker
...
**Difficulty:** Intermediate–Advanced &nbsp;|&nbsp; **Duration:** ~2.5 hours &nbsp;|&nbsp; **Track:** Big Data / DevOps
...
- [⚡ Task 1: Pull and Run Apache Spark Docker Image](#-task-1-pull-and-run-apache-spark-docker-image)
- [🧱 Task 2: Set Up Spark with Master and Worker Nodes](#-task-2-set-up-spark-with-master-and-worker-nodes)
- [📤 Task 3: Submit Spark Jobs to the Cluster](#-task-3-submit-spark-jobs-to-the-cluster)
- [💾 Task 4: Configure Persistent Storage for Spark Data](#-task-4-configure-persistent-storage-for-spark-data)
- [🧩 Task 5: Use Docker Compose to Manage the Spark Cluster](#-task-5-use-docker-compose-to-manage-the-spark-cluster)
...
| # | Objective |
|---|-----------|
| 1 | Pull and run Apache Spark Docker images for distributed data processing |
| 2 | Set up a Spark cluster with master and worker nodes using Docker |
| 3 | Submit and execute Spark jobs on a containerized cluster |
| 4 | Configure persistent storage for Spark data and applications |
| 5 | Use Docker Compose to manage and orchestrate a complete Spark cluster |
| 6 | Understand the fundamentals of running big data workloads in containers |
...
|----------|-------------|
| 🐳 Conceptual | Basic understanding of Docker concepts (containers, images, volumes) |
| 💻 Skills | Familiarity with command-line interface operations |
| 🧠 Conceptual | Basic knowledge of distributed computing concepts |
| 📄 Format | Understanding of YAML file structure for Docker Compose |
| ⚡ Spark | No prior Apache Spark experience required — we'll cover the basics |

---

## 🖥️ Lab Environment Setup

> **☁️ Al Nafi Cloud Machine Access**
> This lab uses Al Nafi's pre-configured Linux-based cloud machines. Simply click **Start Lab** to access your environment — no need to build your own VM or install Docker, everything is ready to use.
...
### ▶️ Subtask 1.2: Run a Basic Spark Container

Let's start with a simple Spark container to understand the basics.

```bash
# ▶️ Run Spark in interactive mode to explore
...
> 🖥️ The Spark Master UI will be available at **http://localhost:8080**

### ⚙️ Subtask 2.3: Start Spark Worker Nodes

```bash
# ⚙️ Start first worker node
docker run -d \
  --name spark-worker-1 \
  --network spark-network \
  -p 8081:8081 \
...
# TODO: Add a third worker with a different SPARK_WORKER_CORES value to compare scheduling
```
...
```python
# 🐍 Create a Python Spark application (wordcount.py)
...
def main():
    ...
    # ✂️ Split lines into words and count
...
# TODO: Add a --output-path arg so results can be written to a file instead of just printed
```
...
# 🖥️ View running applications in Spark UI
...
```bash
# ⏹️ Stop and remove existing containers
...
# ⚙️ Start workers with persistent storage
docker run -d \
  --name spark-worker-1 \
  --network spark-network \
  -p 8081:8081 \
...
# TODO: Kill and recreate spark-master to prove /data survives via the named volume
```
...
```yaml
# 🧩 Create Docker Compose file (~/spark-lab/docker-compose.yml)
version: '3.8'
...
# TODO: Add SPARK_WORKER_MEMORY/CORES as .env-driven variables for easy resizing
```
...
```python
# 📊 Create advanced analytics application (data_analytics.py)
from pyspark.sql import SparkSession
...
# TODO: Add a 4th analysis stage computing average order value per category
```
...
<details>
<summary>❗ Issue 1: Container Connection Problems</summary>
...
<details>
<summary>❗ Issue 3: Memory Issues</summary>

```bash
# 📊 Check container resource usage
docker stats

# ⚙️ Adjust memory settings in Docker Compose
...
<details>
<summary>❗ Issue 4: Volume Mount Problems</summary>
...
| ✅ | Accomplishment |
|----|-----------------|
| ⚡ | **Mastered Spark Containerization** — pulled and ran Apache Spark Docker images, understanding how big data frameworks can be containerized for easier deployment and management |
| 🧱 | **Built a Distributed Cluster** — set up a complete Spark cluster with master and worker nodes using Docker containers, demonstrating scalable distributed computing environments |
...
- ☁️ **Cloud-Ready Deployments** — these skills directly translate to running Spark on Kubernetes and cloud container services

### 🌍 Real-World Applications

The skills you've learned apply to:

- 🏗️ **Data Engineering Pipelines** — processing large datasets in production environments
...
- ☁️ **Cloud Migration** — moving on-premises big data workloads to containerized cloud platforms
...
- ☸️ Learn about Spark on Kubernetes for production deployments
- ⚙️ Study performance tuning and optimization techniques for Spark clusters
