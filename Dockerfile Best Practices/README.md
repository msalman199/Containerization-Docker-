...
### Writing Secure, Efficient, and Production-Ready Dockerfiles
...
- [🏛️ Task 1: Use Official Base Images](#️-task-1-use-official-base-images)
- [📦 Task 2: Minimize the Number of Layers](#-task-2-minimize-the-number-of-layers)
- [⚡ Task 3: Optimize Caching to Speed Up Builds](#-task-3-optimize-caching-to-speed-up-builds)
- [🏗️ Task 4: Use Multi-Stage Builds for Production-Ready Images](#️-task-4-use-multi-stage-builds-for-production-ready-images)
- [🔒 Task 5: Follow Docker Security Best Practices](#-task-5-follow-docker-security-best-practices)
- [🚀 Task 6: Complete Best Practices Implementation](#-task-6-complete-best-practices-implementation)
...
| # | Skill |
|---|-------|
| 1️⃣ | Understand and implement Docker best practices for writing efficient Dockerfiles |
| 2️⃣ | Use official base images to ensure security and reliability |
| 3️⃣ | Minimize Docker layers to reduce image size and improve performance |
| 4️⃣ | Optimize Docker build caching to speed up the build process |
| 5️⃣ | Create multi-stage builds for production-ready, lightweight images |
| 6️⃣ | Implement Docker security best practices including non-root user configuration |
| 7️⃣ | Build secure, efficient, and maintainable Docker images following industry standards |
...
| 🎓 Prior Labs | Completed previous Docker fundamentals labs |
...
## 🏛️ Task 1: Use Official Base Images
...
# 🗂️ Create subdirectories for different examples
...
app.get('/', (req, res) => {
  res.json({
    message: 'Hello from Dockerfile Best Practices Lab!',   // 👋 greeting
...
app.get('/health', (req, res) => {
  res.json({ status: 'healthy' });   // 💓 health probe
});

app.listen(port, '0.0.0.0', () => {
  console.log(`App listening at http://0.0.0.0:${port}`);   // ✅ ready
});
EOF
```
...
# ▶️ Define the command to run the application
CMD ["npm", "start"]
EOF
```
...
# ⚠️ Using generic Ubuntu image (NOT RECOMMENDED)
FROM ubuntu:latest
...
```bash
# 🏗️ Build image with official base
docker build -f Dockerfile.official -t demo-app:official .

# 🏗️ Build image with generic base
...
> 📝 `# TODO:` Repeat this comparison with a base image for your own application's language runtime and record the size delta.
...
# ⚠️ Each RUN instruction creates a new layer
RUN apk update
...
# ⬇️ Install dependencies and clean up in single layer
RUN npm install --only=production && \
...
> 📝 `# TODO:` Count the layers in your own Dockerfiles and look for `RUN` instructions that could be chained together.
...
# ⚠️ Copying everything first (POOR PRACTICE)
COPY . .
...
# ✏️ Make a small change to application code
...
# ✏️ Make same change and rebuild
echo "console.log('Cache test modification');" >> app.js
echo "=== Cache-poor version rebuild ==="
time docker build -f Dockerfile.cache-poor -t demo-app:cache-poor .
```

> 📝 `# TODO:` Time your own project's rebuild before and after reordering `COPY`/`RUN` instructions to quantify the cache win.

---

## 🏗️ Task 4: Use Multi-Stage Builds for Production-Ready Images
...
app.get('/health', (req, res) => {
  res.json({ status: 'healthy', uptime: process.uptime() });   // 💓 health probe
});

app.listen(port, '0.0.0.0', () => {
  console.log(`Multi-stage demo app listening at http://0.0.0.0:${port}`);   // ✅ ready
});
EOF
```
...
# ⚠️ Install ALL dependencies (including dev dependencies)
RUN npm install
...
# ⬇️ Install ALL dependencies (needed for building)
RUN npm install
...
> 📝 `# TODO:` Apply the builder/production split to a project with a real transpile or compile step and confirm the runtime image excludes `devDependencies`.
...
- 🛡️ Minimizing attack surface
- 🏷️ Using specific image tags
...
# 🏷️ Use specific version tag instead of 'latest'
...
# 🛎️ Use dumb-init to handle signals properly
...
# ⚠️ Using latest tag (NOT RECOMMENDED)
FROM node:latest

# ⚠️ Running as root user (SECURITY RISK)
WORKDIR /app

# ⚠️ Copying everything without ownership consideration
COPY . .

# ⚠️ Installing with elevated privileges
RUN npm install

# ⚠️ Exposing privileged port (NOT RECOMMENDED)
EXPOSE 80
...
# 🏷️ Use specific, minimal base image
FROM node:18.17.0-alpine3.18
...
# ⬇️ Install dependencies as root, then clean up
RUN npm install --only=production && \
    npm cache clean --force && \
    rm -rf /tmp/* /root/.npm
...
# 🛎️ Use dumb-init for proper signal handling
ENTRYPOINT ["dumb-init", "--"]
...
> 📝 `# TODO:` Run a real vulnerability scanner (Trivy or Grype) against your production image and address any high-severity findings.
...
# 🏗️ Multi-stage build for production-ready image
# Stage 1: Build stage
...
# ⬇️ Install all dependencies (including dev dependencies for building)
...
# 🛎️ Use tini for proper signal handling
ENTRYPOINT ["tini", "--"]
...
> 📝 `# TODO:` Fold your own application into `Dockerfile.production` and repeat the size/layer/security comparison end-to-end.
...
<details>
<summary>⚡ Issue 1: Build Cache Not Working</summary>
...
<details>
<summary>💓 Issue 3: Health Check Failures</summary>
...
<details>
<summary>📦 Issue 4: Large Image Sizes</summary>
...
```bash
# 1️⃣ Verify all images are built
docker images | grep demo-app

# 2️⃣ Check that production image uses non-root user
docker run --rm demo-app:production whoami

# 3️⃣ Verify health check is configured
docker inspect demo-app:production | grep -A 10 "Healthcheck"

# 4️⃣ Test multi-stage build size efficiency
docker images --format "table {{.Repository}}:{{.Tag}}\t{{.Size}}" | grep -E "(single-stage|multi-stage)"

# 5️⃣ Verify security best practices
...
|---|---|
| 🏛️ Official base images | Images use `node:18-alpine` or similar official images |
| 📦 Layer optimization | Optimized images have fewer layers than unoptimized versions |
| ⚡ Caching | Second builds are significantly faster when only application code changes |
| 🏗️ Multi-stage builds | Production images are smaller than single-stage builds |
| 🔒 Security | Applications run as non-root users with proper permissions |
...
- 🏛️ **Official Base Images** — You learned to use official, maintained base images that provide security updates and optimized configurations, resulting in more reliable and secure containers
...
- 🏗️ **Multi-Stage Builds** — You created production-ready images that separate build-time and runtime dependencies, resulting in smaller, more secure final images
...
- ☸️ Learn about container orchestration platforms like Kubernetes
