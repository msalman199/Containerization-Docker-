...
### Running Apache Hadoop (HDFS + YARN) in a Multi-Container Docker Environment
...
![YARN](https://img.shields.io/badge/YARN-66CCFF?style=for-the-badge&logo=apachehadoop&logoColor=black)
![MapReduce](https://img.shields.io/badge/MapReduce-66CCFF?style=for-the-badge&logo=apachehadoop&logoColor=black)
...
![Track](https://img.shields.io/badge/Track-Big%20Data%20%2F%20DevOps-blueviolet?style=for-the-badge)
...
- [1️⃣ Task 1: Set Up Docker Containers for Hadoop (HDFS and YARN)](#1️⃣-task-1-set-up-docker-containers-for-hadoop-hdfs-and-yarn)
- [2️⃣ Task 2: Start a Multi-Container Hadoop Environment with Docker Compose](#2️⃣-task-2-start-a-multi-container-hadoop-environment-with-docker-compose)
- [3️⃣ Task 3: Interact with the Hadoop Distributed File System (HDFS)](#3️⃣-task-3-interact-with-the-hadoop-distributed-file-system-hdfs)
- [4️⃣ Task 4: Submit Jobs to the YARN Cluster](#4️⃣-task-4-submit-jobs-to-the-yarn-cluster)
- [5️⃣ Task 5: Explore Data Processing in the Hadoop Ecosystem](#5️⃣-task-5-explore-data-processing-in-the-hadoop-ecosystem)
...
| # | Objective |
|---|-----------|
| 1 | Set up and configure Docker containers for Apache Hadoop components (HDFS and YARN) |
| 2 | Deploy a multi-container Hadoop environment using Docker Compose |
| 3 | Interact with the Hadoop Distributed File System (HDFS) through containerized interfaces |
| 4 | Submit and monitor MapReduce jobs to a YARN cluster running in Docker containers |
| 5 | Understand the fundamentals of big data processing using the Hadoop ecosystem in containerized environments |
| 6 | Troubleshoot common issues when running Hadoop in Docker containers |
...
| 🐳 Docker concepts | Familiarity with containers, images, and volumes |
| 🌐 Distributed systems | Basic knowledge of distributed systems concepts |
| 📁 File systems | Understanding of file systems and data processing workflows |
| 💻 Terminal access | Access to a terminal or command prompt |
...
> 🖱️ **Good News!** Al Nafi provides pre-configured Linux-based cloud machines with all necessary tools already installed. Simply click **Start Lab** to access your ready-to-use environment — no need to build your own VM or install Docker manually.
...
## 1️⃣ Task 1: Set Up Docker Containers for Hadoop (HDFS and YARN)
...
Create the core Hadoop configuration files that will be mounted into the containers.

```xml
<!-- ⚙️ config/hadoop/core-site.xml -->
...
```xml
<!-- 🗺️ config/hadoop/mapred-site.xml -->
<?xml version="1.0" encoding="UTF-8"?>
<?xml-stylesheet type="text/xsl" href="configuration.xsl"?>
<configuration>
    <property>
...
## 2️⃣ Task 2: Start a Multi-Container Hadoop Environment with Docker Compose
...
services:
  namenode:                          # 🧠 Master node — tracks the filesystem namespace
  ...
  resourcemanager:                   # 🧭 YARN's cluster resource scheduler
    image: apache/hadoop:3.3.6
  ...
  nodemanager:                       # ⚙️ Runs and manages containers per node
    image: apache/hadoop:3.3.6
  ...
  historyserver:                     # 📜 Archives completed job history
    image: apache/hadoop:3.3.6
...
```bash
# ▶️ Start all services in detached mode
docker-compose up -d
...
> ⏳ Wait for all services to start (this may take 2–3 minutes) before verifying.
...
## 3️⃣ Task 3: Interact with the Hadoop Distributed File System (HDFS)
...
> 📝 **Note:** Open your web browser and navigate to `http://localhost:9870` to explore the HDFS web interface — browse the filesystem, check cluster health, and view detailed cluster information.

---

## 4️⃣ Task 4: Submit Jobs to the YARN Cluster
...
```bash
# ▶️ Still inside the namenode container
...
> 📝 Open your browser and navigate to `http://localhost:8088` to explore the YARN ResourceManager interface, where you can:
...
## 5️⃣ Task 5: Explore Data Processing in the Hadoop Ecosystem

### 🔹 Subtask 5.1: Understand HDFS Block Storage

```bash
# ⚙️ Inside the namenode container
...
# 🏷️ View cluster metrics
yarn cluster -list-node-labels

# 💾 Check HDFS space usage
hdfs dfs -df -h

# 🗺️ View detailed cluster information
...
```bash
# 🗑️ Clean up output directories for reuse
...
<details>
<summary>🟠 Issue 2: HDFS Safe Mode</summary>

```bash
# 🔍 Check safe mode status
docker exec hadoop-namenode hdfs dfsadmin -safemode get

# ⚠️ Force leave safe mode (use with caution)
...
<details>
<summary>🟡 Issue 3: Web UI Not Accessible</summary>
...
<details>
<summary>🟢 Issue 4: Permission Issues</summary>
...
|-------------|-------------|
| 🧩 Container Orchestration | Set up a complete multi-container Hadoop cluster using Docker Compose, showing how containerization simplifies big data infrastructure deployment |
| 💾 HDFS Mastery | Learned to interact with the Hadoop Distributed File System, understanding how data is stored, replicated, and managed across distributed nodes |
| 🧭 YARN Resource Management | Successfully submitted and monitored MapReduce jobs through YARN, gaining hands-on experience with distributed computing resource management |
| 📊 Big Data Processing | Executed various MapReduce jobs — word count, Pi estimation, and terasort — covering different patterns of distributed data processing |
| 🔗 Ecosystem Integration | Explored how HDFS, YARN, and MapReduce work together in the Hadoop ecosystem |

### 💡 Why This Matters

- **🏭 Industry Relevance** — Hadoop remains a cornerstone technology for big data processing in enterprise environments
- **📦 Containerization Benefits** — Running Hadoop in Docker provides consistency, portability, and easier deployment across environments
- **📈 Scalability Understanding** — Practical experience with distributed systems concepts that apply to modern big data architectures
- **⚙️ DevOps Skills** — Bridges the gap between big data technologies and modern containerization practices

### 🔮 Next Steps

- 🐝 Explore more advanced Hadoop ecosystem tools like Apache Hive, Apache Spark, or Apache HBase
- ☸️ Learn about Kubernetes orchestration for Hadoop clusters
- ☁️ Investigate cloud-native big data solutions that build upon these foundational concepts
- 📊 Practice with larger datasets and more complex MapReduce algorithms

> This hands-on experience with Hadoop in Docker containers gives you valuable skills for both the Docker Certified Associate (DCA) certification and real-world big data engineering scenarios! 🚀
